...
Groupe::~Groupe() {
	...
	for (int j = 0; j < transferts_.size(); j++) {
...
Groupe& Groupe::operator+=(Utilisateur* utilisateur) {
	if ((dynamic_cast<UtilisateurRegulier*>(utilisateur))) {
		if ((dynamic_cast<UtilisateurRegulier*>(utilisateur))->getPossedeGroupe() != true) {
			...
			cout << utilisateur->getNom()
				<< " nest pas un utilisateur premmium et ne peut ps etre ajoute car il est deja dans un groupe" << endl;
		}
	}
		...
		if ((dynamic_cast<UtilisateurPremium*>(utilisateur))->getJoursRestants() != 0) {
			...
			cout << utilisateur->getNom() << " doit renouveler son abonnemment premium" << endl;
		}
	}
	return *this;
	//Other way ?
